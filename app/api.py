...
from app.config import DEFAULT_NAMESPACE, DATA_DIR, ENABLE_CACHE, ENABLE_HYBRID_SEARCH
...
# Initialize managers
multi_retriever = EnhancedMultiNamespaceRAGRetriever()
...
# Enhanced Query Endpoints with Caching and Hybrid Search

class EnhancedQueryRequest(BaseModel):
    query: str = Field(..., description="The search query text")
    top_k: int = Field(default=5, ge=1, le=50, description="Number of results to return")
    score_threshold: float = Field(default=0.0, ge=0.0, le=1.0, description="Minimum similarity score")
    use_hybrid: Optional[bool] = Field(default=None, description="Force hybrid search on/off")
...
@app.post("/v2/query/{namespace}", response_model=EnhancedQueryResponse)
async def enhanced_query_namespace(
    namespace: str,
    request: EnhancedQueryRequest,
    current_user: User = Depends(get_current_active_user)
):
    """Enhanced query with caching and hybrid search support"""
    start_time = asyncio.get_event_loop().time()
    ...
    try:
        ...
        retriever = multi_retriever.get_retriever(namespace)
        ...
        if request.use_cache and ENABLE_CACHE:
            ...
            results = retriever._execute_search(
                ...
                score_threshold=request.score_threshold,
                filters=request.filters or {},
                use_hybrid=request.use_hybrid or ENABLE_HYBRID_SEARCH
            )
        ...
        for result in results:
            enhanced_results.append(EnhancedQueryResult(
                text=result['text'],
                score=result['score'],
        ...
        logger.error(f"Enhanced namespace query failed: {e}")
        raise HTTPException(status_code=500, detail=f"Query failed: {str(e)}")

@app.post("/v2/query/cross-namespace", response_model=EnhancedQueryResponse)
async def enhanced_cross_namespace_query(
    request: EnhancedQueryRequest,
    namespaces: List[str] = Query(default=None, description="Specific namespaces to search"),
    current_user: User = Depends(get_current_active_user)
):
    """Enhanced cross-namespace search with hybrid capabilities"""
    start_time = asyncio.get_event_loop().time()
    ...
    try:
        ...
        # Filter namespaces based on user access
        available_namespaces = namespace_manager.list_namespaces()
        
        if namespaces:
            # Check access to requested namespaces
            accessible_namespaces = []
            for ns in namespaces:
                if require_namespace_access(ns, current_user) and ns in available_namespaces:
                    accessible_namespaces.append(ns)
            
            if not accessible_namespaces:
                raise HTTPException(status_code=403, detail="No accessible namespaces specified")
            
            search_namespaces = accessible_namespaces
        else:
            # Use all accessible namespaces
            search_namespaces = [ns for ns in available_namespaces 
                               if require_namespace_access(ns, current_user)]
          # Perform cross-namespace search
        results = multi_retriever.query_cross_namespace(
        ...
        for result in results:
            enhanced_results.append(EnhancedQueryResult(
                ...
                metadata=result['metadata'],
                search_type=result.get('search_type', 'cross_namespace')
            ))
        ...
        return EnhancedQueryResponse(
            ...
            total_results=len(enhanced_results),
            processing_time_ms=processing_time,
            namespaces_searched=search_namespaces,
            search_type='cross_namespace_hybrid' if ENABLE_HYBRID_SEARCH else 'cross_namespace_vector'
        )
        
    except Exception as e:
        logger.error(f"Enhanced cross-namespace query failed: {e}")
        raise HTTPException(status_code=500, detail=f"Cross-namespace query failed: {str(e)}")
...
@app.post("/cache/invalidate/{namespace}")
async def invalidate_namespace_cache(
    namespace: str,
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        await cache_manager.invalidate_namespace_cache(namespace)
...
async def clear_all_cache(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        # Clear cache for all namespaces
        namespaces = namespace_manager.list_namespaces()
        for namespace in namespaces:
            await cache_manager.invalidate_namespace_cache(namespace)
        
        return {
            "message": f"Cache cleared for all {len(namespaces)} namespaces",
...
@app.get("/analytics/namespace/{namespace}")
async def get_namespace_analytics(
    namespace: str,
    current_user: User = Depends(get_current_active_user)
):
    """Get comprehensive analytics for a namespace"""
    try:
        if not can_read(current_user):
            raise HTTPException(status_code=403, detail="Insufficient permissions")
        ...
        logger.error(f"Error getting namespace analytics: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get analytics: {str(e)}")

@app.get("/analytics/search/{namespace}")
async def get_search_analytics(
    namespace: str,
    current_user: User = Depends(get_current_active_user)
):
    """Get search-specific analytics for a namespace"""
    try:
        if not can_read(current_user):
            raise HTTPException(status_code=403, detail="Insufficient permissions")
        ...
        # Get search analytics from hybrid engine
        engine = hybrid_search_engine.get_engine(namespace)
